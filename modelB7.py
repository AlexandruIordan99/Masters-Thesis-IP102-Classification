...
from keras._tf_keras.keras.applications import EfficientNetB7
import argparse
...
model_B7 = EfficientNetB7(include_top=True,
                          weights=None,
                          classes = NUM_CLASSES,
                          input_shape=(IMG_SIZE, IMG_SIZE, 3))

model_B7.compile(optimizer='adam',loss='sparse_categorical_crossentropy',metrics=['accuracy'])
model_B7.summary()
...
def main():
    ...
    if args.model:
        ...
        model_B7.load_weights(args.model)
        model_B7.compile(optimizer='adam',loss='sparse_categorical_crossentropy',metrics=['accuracy'])
        history = model_B7.predict(train_ds, validation_data=test_ds, epochs=epochs)
    else:
        print(f"Training {args.save}...")
        model_B7.compile(optimizer='adam',loss='sparse_categorical_crossentropy',metrics=['accuracy'])
        hist_1= model_B7.fit(train_ds, epochs=epochs, validation_data=val_ds, batch_size= 64)

    if args.save:
        print(f"Saving {args.save}...")
        model_B7.save_weights(args.save)
