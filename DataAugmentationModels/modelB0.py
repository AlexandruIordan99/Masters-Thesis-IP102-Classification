...
#To train and evaluate all models, simply replace the EfficientNet model version or use the other files in this folder


# Paths to datasets - replace these with your own paths
...
def build_model(num_classes):
    ...
    optimizer = keras.optimizers.Adam(learning_rate=0.01)  #kept as in Doan 2023
    model.compile(
        optimizer=optimizer,
...
def unfreeze_model(model):
    #Unfreeze top 15 layers
    for layer in model.layers[-15:]:
        if not isinstance(layer, layers.BatchNormalization):
            layer.trainable = True

    optimizer = keras.optimizers.Adam(learning_rate=0.0001) #kept as in Doan 2023
    model.compile(
        optimizer=optimizer,
        loss="categorical_crossentropy",
        metrics=["accuracy",
...
epochs = 30  # @param {type: "slider", min:4, max:10}  #Same as in Doan 2023
hist = model.fit(train_ds, epochs=epochs, validation_data=val_ds)
plot_hist(hist)
model.save_weights("/home/jordan/modelV1B0dataAug.weights.h5")
model.evaluate(test_ds)
model.load_weights("/home/jordan/modelV1B0dataAug.weights.h5")
model.evaluate(test_ds)
