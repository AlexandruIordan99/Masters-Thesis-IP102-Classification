...
def main():
    ...
    if args.model:
        ...
        model_B0.load_weights(args.model)
        history = model_B0.predict(train_ds, validation_data=test_ds, epochs=epochs)
    else:
        print(f"Training {args.save}...")
        hist_1= model_B0.fit(train_ds, epochs=epochs, validation_data=val_ds, batch_size= 64)

    if args.save:
        print(f"Saving {args.save}...")
        model_B0.save_weights(args.save)
