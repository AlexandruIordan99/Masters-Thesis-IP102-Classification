...
model_V2L = EfficientNetV2L(include_top=True,
                            weights=None,
                            classes = NUM_CLASSES,
                            input_shape=(IMG_SIZE, IMG_SIZE, 3))

model_V2L.compile(optimizer='adam',loss='sparse_categorical_crossentropy',metrics=['accuracy'])
model_V2L.summary()
...
def main():
    ...
    if args.model:
        ...
        model_V2L.load_weights(args.model)
        model_V2L.compile(optimizer='adam',loss='sparse_categorical_crossentropy',metrics=['accuracy'])
        history = model_V2L.predict(train_ds, validation_data=test_ds, epochs=epochs)
    else:
        print(f"Training {args.save}...")
        model_V2L.compile(optimizer='adam',loss='sparse_categorical_crossentropy',metrics=['accuracy'])
        hist_1= model_V2L.fit(train_ds, epochs=epochs, validation_data=val_ds, batch_size= 64)

    if args.save:
        print(f"Saving {args.save}...")
        model_V2L.save_weights(args.save)
