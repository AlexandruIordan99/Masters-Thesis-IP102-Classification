...
model_B1 = EfficientNetB1(include_top=True,
                          weights=None,
                          classes=NUM_CLASSES,
                          input_shape=(IMG_SIZE, IMG_SIZE, 3))

model_B1.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
model_B1.summary()
...
def main():
    ...
    if args.model:
        ...
        model_B1.load_weights(args.model)
        model_B1.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        history = model_B1.predict(test_ds)
    else:
        print(f"{path_to_train_set}")
        print(f"Training {args.save}...")
        model_B1.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        hist_1 = model_B1.fit(train_ds, epochs=epochs, validation_data=val_ds, batch_size=64)

    if args.save:
        print(f"Saving {args.save}...")
        model_B1.save_weights(args.save)
