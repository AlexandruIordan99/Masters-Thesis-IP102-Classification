...
def dataset_to_numpy_in_batches(dataset, batch_size):
    ...
    for img_batch, label_batch in dataset:
        ...
        if current_batch_size >= batch_size:
            ...
            # Reset images, labels, and the current batch IMG_SIZE for the next batch
...
train_ds = tf.data.Dataset.from_generator(
    ...
    output_signature=(tf.TensorSpec(shape=(None, 240, 240, 3), dtype=tf.float32),  #CHANGE RESOLUTION BASED ON MODEL
...
plot_hist(hist)
model.save_weights("/home/jordan/modelV1B1GSMOTE.weights.h5")
