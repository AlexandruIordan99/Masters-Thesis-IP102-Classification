...
from keras._tf_keras.keras.applications import EfficientNetB4
import argparse
...
model_B4 = EfficientNetB4(include_top=True,
                          weights=None,
                          classes = NUM_CLASSES,
...
def main():
    ...
    if args.model:
        ...
        model_B4.load_weights(args.model)
        model_B4.compile(optimizer='adam',loss='sparse_categorical_crossentropy',metrics=['accuracy'])
        history = model_B4.predict(train_ds, validation_data=test_ds, epochs=epochs)
    else:
        print(f"Training {args.save}...")
        model_B4.compile(optimizer='adam',loss='sparse_categorical_crossentropy',metrics=['accuracy'])
        hist_1= model_B4.fit(train_ds, epochs=epochs, validation_data=val_ds, batch_size= 64)

    if args.save:
        print(f"Saving {args.save}...")
        model_B4.save_weights(args.save)
