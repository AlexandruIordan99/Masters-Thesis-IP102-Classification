...
while not converged:
    ...
    # Slice the confusion matrix to show only the first 20 classes
    cm_normalized_subset = cm_normalized[:20, :20]

    # Plot the sliced confusion matrix
    sns.heatmap(cm_normalized_subset, annot=True, fmt='.2f', cmap='Blues')
    plt.ylabel('True label (First 20 classes)')
    plt.xlabel('Predicted label (First 20 classes)')
